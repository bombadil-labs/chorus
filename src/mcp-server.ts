...
export const TOOLS = [
  ...
  },
  {
    name: "remember",
    description:
      "Assert a belief as a signed claim: about (entity id), attribute (property name), value (primitive, or {entity} for a typed reference — see the value description), optional kind (open vocabulary — core: observation|fact|preference|task|claim), confidence (0..1), source, speaker.",
    inputSchema: {
      type: "object",
      properties: {
        about: { type: "string" },
        ...
        kind: {
          type: "string",
          description:
            "What KIND of belief this is. Core vocabulary: 'observation' (something noticed), " +
            "'fact' (a world-truth), 'preference' (about the principal), 'task' (something " +
            "open), 'claim' (an interpretation or thesis being ADVANCED — distinct from a " +
            "passive observation). The vocabulary is OPEN, like every id here: mint a new " +
            "kind when the core five genuinely don't fit, reuse before minting (typos " +
            "fragment), and know that briefing surfaces lean on 'preference' and 'task' " +
            "specifically.",
        },
          ...
          ...(kindPtr?.target.kind === "primitive"
            ? {
                kind: String(kindPtr.target.value) as
                  "observation" | "fact" | "preference" | "task",
              }
            : {}),
        }),
        {
...
if (
  ...
  if (packPath !== undefined) {
    ...
    // Default: the shared durable store — many concurrent sessions, one world. Path + backend
    // resolve together (CHORUS_STORE / CHORUS_STORE_BACKEND, with legacy-jsonl continuity).
    const { path: storePath, kind } = resolveEnvStore();
    const store = createBackend(storePath, kind);
