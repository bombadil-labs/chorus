...
import { availableDriver, createBackend, type StoreBackend } from "../src/store-tier.js";
...
// Either sqlite-family driver serves this suite (identical file + indexes); skip loudly only
// when NEITHER exists (an old Node whose optional better-sqlite3 build was skipped).
...
const sqlite: StoreBackend | undefined = canSqlite
  ? createBackend(sqlitePath, availableDriver("sqlite"))
  : undefined;
afterAll(() => {
  sqlite?.close?.();
...
describe.skipIf(!canSqlite)(
  "chorus SQLite tier: indexed reads reproduce the scan, and beat it",
  () => {
    ...
    it("backlinks: indexed (SQLite) == scan (JSONL), and the index is faster over many queries", () => {
      ...
      const ROUNDS = 3;
      const time = (store: StoreBackend): number => {
