...
hindsight**: a decision pins _what was known when it was made_, and everything learned afterward is
...
wrong `decided-at`), that is the only case for `revise`; a _changed mind_ is an appended `review-note`.
...
- **`decide {about, intent, attribute?}`** — call this **at the moment of deciding**. It resolves
  `about` _now_ and pins one signed record of the basis: the instant, the trust policy, the resolved
