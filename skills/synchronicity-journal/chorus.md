...
(base rates, priming, apophenia) is _always_ available for any coincidence, so applying it per-entry
...
_explicitly_ invokes it for a specific entry. Grade entries by **register** (the kind of meaning),
...
- `concept:<slug>` — a recurring motif, symbol, or theme (`concept:the-number-11`, `concept:water`,
  `concept:doorways`). The nodes that synchronicities _rhyme through_.
...
| `composed-of` | `{entity: "event:x"}` × many                                    | observation | **SET** — the events/entities the coincidence is _made of_. References, not strings. |
...
| `rhyme`       | primitive (prose)                                               | observation | _what_ rhymes — the shape of the echo, in words                                      |
...
**2. "The composed-of structure of a synchronicity."** — what the coincidence is _made of_.
...
**3. Register distribution over time.** — the _texture_ of a season, not a scoreboard. Group by
