...
The MCP tools a Chorus node exposes ([src/mcp-server.ts](../../../src/mcp-server.ts)),
...
- `revise {deltaId, value, reason?}` — retract + re-assert in one move (the fact _changed_).
...
- `retract {deltaId, reason?}` — append a negation (the claim was _wrong_). History stays.
