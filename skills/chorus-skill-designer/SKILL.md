...
1. **Claude-mediated (THE mode for now).** Claude is the runtime. On invocation the skill tells
   ...
   _intent_ (a form submit → a `chorus:write` event / `window.CHORUS_PENDING`), Claude performs the
...
2. **Standalone (optional, later).** The same file reads a `window.CHORUS_ENDPOINT` config (an HTTP
   MCP node URL + token, per [src/mcp-http.ts](../../src/mcp-http.ts)) and
...
- **Reference, don't transcribe.** A value that _names_ something the store could hold beliefs about is
...
- **Pick the right `kind`.** `observation` (something happened / was seen), `fact` (world state),
  ...
  enum lacks (an _interpretation_), file `observation` and note it; don't overload `fact`.
...
4. **Write `SKILL.md`.** Frontmatter `description` must say _when_ to trigger (domain nouns + verbs).
