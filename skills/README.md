...
Chorus ([this repo](../README.md)) is domain-agnostic substrate: every belief is a signed,
content-addressed claim; reads resolve under a trust policy; and **GraphQL is synthesized on demand**
from whatever the store happens to hold ([src/gql.ts](../src/gql.ts)). That
...
the same posture Chorus itself takes toward the format ([CLAUDE.md](../CLAUDE.md)). A skill never
changes how Chorus resolves truth; it only teaches a domain's vocabulary and paints a UI over the
answers.
